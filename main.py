...
class App(ctk.CTk):
    ...
    def cadastrar_produto(self):
        with sqlite3.connect('produtos.db') as conexao:
            ...
            db.execute(
                'create table if not exists produtos(Codigo do Produto text, Nome do Produto text, Descrição do Produto text, Valor do Produto text, Tipo do Produto text);')
            ...
            db.execute('insert into produtos values(?,?,?,?,?)',
                       [self.codigo_produto, self.nome_produto, self.descricao_produto, self.valor_produto,
                        self.tipo_produto])
            conexao.commit()
    ...
    def tela_principal(self):
        ...
        self.title_frame = ctk.CTkFrame(self, width=500, height=50, fg_color='#038bbb')
        self.title_text = ctk.CTkLabel(self.title_frame, text='Cadastro de Produtos', font=("arial bold", 20))
        ...
        self.tipo_do_produto_opt = ctk.CTkOptionMenu(self, values=['Papelaria', 'Informática', 'Eletrônicos'],
                                                     width=240, height=35, fg_color='grey', corner_radius=15)
        self.tipo_do_produto_opt.set('Selecione')

        self.botao_cadastrar = ctk.CTkButton(self, text='Cadastrar', font=('arial bold', 20), height=35, width=260,
                                             command=self.cadastrar_produto)

        # Posicionando Frame
        self.title_frame.pack()
        self.title_text.place(x=150, y=10)
